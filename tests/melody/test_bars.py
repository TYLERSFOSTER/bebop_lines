...
def test_PermutationBar__first_degree(permutation_group, start_idx, value_shift, tonic_degree):
    for permutation in permutation_group:
        present_bar = bars.PermutationBar(permutation, start_idx, start_idx + 2, 0, value_shift, tonic_degree=tonic_degree)

        assert present_bar.first_degree() == permutation[start_idx] + tonic_degree + value_shift
