...
def save_MIDI(phrase : line.PermutationPhrase, use_curve_amplitude=False):
  ...
  for pitch, duration, velocity in zip(degree_phrase, duration_phrase, velocity_phrase):
    ...
    track.append(Message('note_on', note=pitch, velocity=velocity, time=ticks_per_beat * running_clock))
    
    running_clock = running_clock + duration
    track.append(Message('note_off', note=pitch, velocity=velocity, time=ticks_per_beat * running_clock))
