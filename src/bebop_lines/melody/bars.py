...
class PermutationBar():
  ...
  Attributes:
      ...
      value_shift : A constant shift applied to all values of the permutation before converting to MIDI.
      ...
      duration_list : A list of durations (in arbitrary units) corresponding to each index in the permutation.
  """
  def __init__(
    self,
  ...
  def change_shifts(self, new_idx_shift : int, new_value_shift : int) -> None:
    ...
    Args:
        new_idx_shift : New circular index shift to apply to time steps before indexing the permutation.
        new_value_shift : New amount to add to each pitch value in the permutation before mapping to MIDI.
    """
